...
app.config[
  "MAX_CONTENT_LENGTH"
] = int(os.environ.get("MAX_UPLOAD_BYTES", str(5 * 1024 * 1024)))
...
rate_buckets = {}
RATE_LIMITS = {
  "analizar": (int(os.environ.get("ANALYZE_RATE_LIMIT",
  "8")),
  3600),
  "generar_informe": (int(os.environ.get("GENERATE_RATE_LIMIT",
  "20")),
  3600),
}
...
MAX_TOPIC_LENGTH = int(os.environ.get("MAX_TOPIC_LENGTH",
"180"))
MAX_GRADE_LENGTH = int(os.environ.get("MAX_GRADE_LENGTH",
"80"))
...
401
        ...
        if decoded.get("email") and not decoded.get("email_verified", False):
            return jsonify({
  "error": "Verifica tu correo antes de usar esta funcion."
}),
403
...
{}) or {}
            ...
            bucket = [stamp for stamp in rate_buckets.get(key,
  []) if now - stamp < window_seconds
]

            if len(bucket) >= limit:
                return jsonify({
  "error": "Limite de uso alcanzado. Intenta nuevamente mas tarde."
}),
429
...
"script-src 'self' 'unsafe-inline' https://www.gstatic.com https://cdnjs.cloudflare.com https://apis.google.com; ""style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; ""font-src 'self' https://fonts.gstatic.com; ""img-src 'self' data: blob: https://*.googleusercontent.com; ""connect-src 'self' https://identitytoolkit.googleapis.com https://securetoken.googleapis.com https://www.googleapis.com https://apis.google.com; ""frame-src 'self' https://*.firebaseapp.com https://accounts.google.com; ""object-src 'none'; base-uri 'self'; frame-ancestors 'none'"
)
    return response
...
def payload_too_large(error):
    return jsonify({
  "error": "La imagen es demasiado grande. Maximo permitido: 5 MB."
}),
413
...
- fontWeight: 300=trazo delgado,
400=normal,
600=trazo grueso o presionado.
- letterSpacing: espacio entre letras, donde -1=compacta,
0=normal y 3=espaciada.
- jitter: irregularidad visual de letras y palabras,
0=muy uniforme,
2=muy irregular.
...
def generate_content_with_fallback(contents, config, attempts_per_model=2):
    ...
    raise RuntimeError(
        "Gemini esta saturado o sin cuota disponible ahora mismo. ""Espera un minuto y vuelve a intentar. Ultimo error: " + str(last_error)
    )
...
def sanitize_result(result):
  ...
  "600"
    ...
    ink = result.get("ink")
    font_weight = str(result.get("fontWeight",
"400"))
  ...
  "textSize": clamp_number(result.get("textSize"),
  16,
  32,
  22, int),
  "lineH": clamp_number(result.get("lineH"),
  24,
  48,
  32, int),
  "fontWeight": font_weight if font_weight in valid_weights else "400",
  "letterSpacing": clamp_number(result.get("letterSpacing"),
  -1,
  3,
  0, float),
  "jitter": clamp_number(result.get("jitter"),
  0,
  2,
  0.8, float),
  "wordSpacing": clamp_number(result.get("wordSpacing"),
  0,
  6,
  2, float),
  "lineRotation": clamp_number(result.get("lineRotation"),
  0,
  1.2,
  0.25, float),
  "baselineWave": clamp_number(result.get("baselineWave"),
  0,
  3,
  0.8, float),
  "confidence": clamp_number(result.get("confidence"),
  0,
  1,
  0.8, float),
  "description": str(result.get("description") or "Estilo detectado automaticamente")[
    : 90
  ],
}
...
def analizar():
    if client is None:
        return jsonify({
  "error": "Falta configurar la variable de entorno GEMINI_API_KEY"
}),
500
...
415
        ...
        logger.warning("gemini_analysis_json_error=%s raw_prefix=%s", error, raw[
  : 160
])
        return jsonify({
  "error": "La IA devolvio una respuesta invalida. Intenta nuevamente."
}),
502
    except Exception as error:
        logger.exception("analysis_error=%s", error)
        return jsonify({
  "error": "No se pudo analizar la imagen en este momento."
}),
502
...
@app.route("/generar-informe", methods=[
  "POST"
])
@require_auth
...
500
    ...
    try:
        ...
        if not tema:
            return jsonify({
  "error": "Falta el tema del informe"
}),
400
        if len(tema) > MAX_TOPIC_LENGTH:
            return jsonify({
  "error": f"El tema no puede superar {MAX_TOPIC_LENGTH} caracteres."
}),
400
        if len(grado) > MAX_GRADE_LENGTH:
            return jsonify({
  "error": f"El grado o nivel no puede superar {MAX_GRADE_LENGTH} caracteres."
}),
400
  ...
  "largo": "1000 a 1300 palabras",
        ...
        target_length = extension_map.get(extension, extension_map[
  "medio"
])
...
"gemini-2.5-flash-lite")
        ...
        logger.warning("gemini_report_json_error=%s raw_prefix=%s", error, raw[
  : 160
])
        return jsonify({
  "error": "La IA devolvio una respuesta invalida. Intenta nuevamente."
...
@app.route("/css/<path:filename>", methods=[
  "GET"
])
...
@app.route("/js/<path:filename>", methods=[
  "GET"
])
